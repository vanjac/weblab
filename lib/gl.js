...
const $gl = (() => {
	 ...
	 * @returns {WebGLShader}
	 */
	self.createShader = function(gl, type, source, defines) {
	 ...
	 * @param {string[]} defines
	 */
	self.fragShader = function(gl, source, ...defines) {
		return self.createShader(gl, gl.FRAGMENT_SHADER, source, defines)
	}
	 ...
	 * @param {string[]} defines
	 */
	self.vertShader = function(gl, source, ...defines) {
		return self.createShader(gl, gl.VERTEX_SHADER, source, defines)
	}
	 ...
	 */
	self.createProgram = function(gl, shaders) {
		let program = gl.createProgram()
		for (let shader of shaders) {
			gl.attachShader(program, shader)
		}

		for (let [name, index] of Object.entries(self.boundAttr)) {
	 ...
	 */
	self.vertexAttribStatic = function(gl, index, data, size, type, normalized = false) {
	 ...
	 */
	self.makeDynamicBuffer = function(gl) {
	 ...
	 * @param {GLenum} usage
	 */
	self.dynamicBufferData = function(gl, dbuf, target, data, usage = gl.STREAM_DRAW) {
	 ...
	 */
	self.makeVertexBufferCache = function(gl) {
	 ...
	 * @param {boolean} normalized
	 */
	self.vertexAttribData = function(cache, index, data, size, type, normalized = false) {
		let {gl} = cache
		let dbuf = cache.arrayBuffers[index]
		if (!dbuf) {
			dbuf = self.makeDynamicBuffer(gl)
			cache.arrayBuffers[index] = dbuf
		}
		self.dynamicBufferData(gl, dbuf, gl.ARRAY_BUFFER, data)
		gl.vertexAttribPointer(index, size, type, normalized, 0, 0)
	}
	 ...
	 */
	self.drawElements = function(cache, mode, indices) {
		let {gl} = cache
		if (!cache.elementBuffer) {
			cache.elementBuffer = self.makeDynamicBuffer(gl)
		}
		self.dynamicBufferData(gl, cache.elementBuffer, gl.ELEMENT_ARRAY_BUFFER, indices)
	 ...
	 */
	self.perspective = function(aspect, near, fy = 1) {
