import * as $gl from './gl.js'

const glConst = WebGLRenderingContext

/**
 * @template {Record<string, boolean>} [O={}]
 * @param {GLenum} type
 * @param {O} defaultOptions
 * @param {string} source
 * @returns {(gl: WebGLRenderingContext, options?: {[P in keyof O]?: boolean}) => WebGLShader}
 */
function shaderFunc(type, defaultOptions, source) {
    return (gl, options = {}) => {
        let defines = ''
        let combined = Object.assign({}, defaultOptions, options)
        for (let [key, value] of Object.entries(combined)) {
            if (value) {
                defines += '#define OPTION_' + key + '\n'
            }
        }
        return $gl.createShader(gl, defines + source, type)
    }
}

export const basicVert = shaderFunc(glConst.VERTEX_SHADER, {},`
    attribute vec4 aPosition;
    attribute vec3 aNormal;
    attribute vec4 aColor;
    attribute vec2 aTexCoord0;

    varying vec4 vPosition;
    varying vec3 vNormal;
    varying vec4 vColor;
    varying vec2 vTexCoord0;

    void main() {
        vPosition = aPosition;
        vNormal = aNormal;
        vColor = aColor;
        vTexCoord0 = aTexCoord0;
        gl_Position = aPosition;
    }
`)

export const transformVert = shaderFunc(glConst.VERTEX_SHADER, {},`
    uniform mat4 uModelMat;
    uniform mat4 uViewMat;
    uniform mat4 uProjMat;

    attribute vec3 aPosition;
    attribute vec3 aNormal;
    attribute vec4 aColor;
    attribute vec2 aTexCoord0;

    varying vec4 vPosition;
    varying vec3 vNormal;
    varying vec4 vColor;
    varying vec2 vTexCoord0;

    void main() {
        vPosition = uModelMat * vec4(aPosition, 1);
        vNormal = aNormal;
        vColor = aColor;
        vTexCoord0 = aTexCoord0;
        gl_Position = uProjMat * uViewMat * vPosition;
    }
`)

export const screenVert = shaderFunc(glConst.VERTEX_SHADER, {}, `
    attribute vec4 aPosition;

    varying vec2 vTexCoord0;

    void main() {
        vTexCoord0 = (aPosition.xy + 1.0) / 2.0;
        gl_Position = aPosition;
    }
`)

export const pointVert = shaderFunc(glConst.VERTEX_SHADER, {}, `
    uniform float uPointSize;

    attribute vec4 aPosition;
    attribute vec4 aColor;

    varying vec4 vPosition;
    varying vec4 vColor;

    void main() {
        vPosition = aPosition;
        vColor = aColor;
        gl_Position = aPosition;
        gl_PointSize = uPointSize;
    }
`)

export const basicFrag = shaderFunc(glConst.FRAGMENT_SHADER, {
    texture: false,
}, `
    precision mediump float;

    varying vec4 vColor;

#ifdef OPTION_texture
    uniform sampler2D sColor;
    varying vec2 vTexCoord0;
#endif

    void main() {
        vec4 color = vColor;
#ifdef OPTION_texture
        color *= texture2D(sColor, vTexCoord0);
#endif
        gl_FragColor = color;
    }
`)
